...
def visualize_and_play_audio_pair(clean_waveform, noisy_waveform, sample_rate=16000):
    """
    Visualizes the waveforms of clean and noisy audio and provides playback options.
    
    Args:
    - clean_waveform (torch.Tensor): The clean audio waveform
    - noisy_waveform (torch.Tensor): The noisy audio waveform
    - sample_rate (int): The sample rate of the audio (default: 16000)
    """
    # Ensure waveforms are on CPU and convert to numpy arrays
    clean_waveform = clean_waveform.cpu().numpy().squeeze()
    noisy_waveform = noisy_waveform.cpu().numpy().squeeze()
    
    # Create time axis
    time = np.arange(0, len(clean_waveform)) / sample_rate
    
    # Plot waveforms
    plt.figure(figsize=(12, 6))
    plt.subplot(2, 1, 1)
    plt.plot(time, clean_waveform)
    plt.title('Clean Audio Waveform')
    plt.ylabel('Amplitude')
    plt.xlabel('Time (s)')
    
    plt.subplot(2, 1, 2)
    plt.plot(time, noisy_waveform)
    ...
    clean_normalized = clean_waveform / np.max(np.abs(clean_waveform))
    noisy_normalized = noisy_waveform / np.max(np.abs(noisy_waveform))
    
    print("Clean Audio:")
    display(Audio(clean_normalized, rate=sample_rate))
    
    print("Noisy Audio:")
    display(Audio(noisy_normalized, rate=sample_rate))

def visualize_and_play_audio_triplet(clean_waveform, noisy_waveform, generated_waveform, sample_rate=16000):
    """
    Visualizes the waveforms of clean, noisy and generated audio and provides playback options.
    
    Args:
    - clean_waveform (torch.Tensor): The clean audio waveform
    - noisy_waveform (torch.Tensor): The noisy audio waveform
    - generated_waveform (torch.Tensor): The generated audio waveform
    - sample_rate (int): The sample rate of the audio (default: 16000)
    """
    # Ensure waveforms are on CPU and convert to numpy arrays
    clean_waveform = clean_waveform.cpu().numpy().squeeze()
    noisy_waveform = noisy_waveform.cpu().numpy().squeeze()
    generated_waveform = generated_waveform.cpu().numpy().squeeze()
    ...
    plt.subplot(3, 1, 3)
    plt.plot(time, generated_waveform)
    ...
    clean_normalized = clean_waveform / np.max(np.abs(clean_waveform))
    noisy_normalized = noisy_waveform / np.max(np.abs(noisy_waveform))
    generated_normalized = generated_waveform/ np.max(np.abs(generated_waveform))
    ...
    display(Audio(generated_normalized, rate=sample_rate))