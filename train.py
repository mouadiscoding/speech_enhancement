...
def train(clean_dir, noisy_dir, batch_size, log_dir, exp_path, ckpt_iter_arg, learning_rate, 
    ...
    _, trainloader = load_CleanNoisyPairDataset(clean_dir, noisy_dir, batch_size=batch_size)
    print('Data loaded')
    ...
    while n_iter < n_iters + 1:
        # for each epoch
        for clean_audio, noisy_audio in trainloader: 
...
def train_with_val(clean_dir, noisy_dir, batch_size, train_split_size, log_dir, exp_path, ckpt_iter_arg, learning_rate, 
          ell_p, ell_p_lambda, n_iters, 
          stft_lambda, iters_per_valid, iters_per_ckpt):    
    # HEADS-UP: removed stft_config from the params
